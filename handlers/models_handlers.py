...
async def callback_setmodel(callback: types.CallbackQuery):
    ...
    if data != "cancel":
        if data in ["meta-llama/Llama-3.2-90B-Vision-Instruct", "Qwen/Qwen2-VL-7B-Instruct"]:
            await DB.update_users_ai_visual_model(user_id=callback.from_user.id, new_ai_visual_model=data)
        else:
            await DB.update_users_ai_model(user_id=callback.from_user.id, new_ai_model=data)
